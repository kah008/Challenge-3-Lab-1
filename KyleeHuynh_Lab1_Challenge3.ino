...
#define TOGGLE_COLOR_KEY 0xFF30CF
#define BRIGHTNESS_UP_KEY 0xFF9867
#define BRIGHTNESS_DOWN_KEY 0xFF38C7
#define CIRCLE_KEY 0xFF18E7
#define CUSTOM_FUNCTION_KEY_A 0xFF22DD
#define CUSTOM_FUNCTION_KEY_B 0xFF02FD
#define CUSTOM_FUNCTION_KEY_C 0xFFC23D
  ...
  if (irrecv.decode(&results)) {
    ...
    if (isOn == true) {
      if (results.value == TOGGLE_COLOR_KEY)      // Toggle active colors
        ...
        circle();
      else if (results.value == CUSTOM_FUNCTION_KEY_A) {
        servoReset();
      }
      else if (results.value == CUSTOM_FUNCTION_KEY_B) {
        servoShort();
        brightnessUp();
      }
      else if (results.value == CUSTOM_FUNCTION_KEY_C) {
        servoLong();
        brightnessDown();
      }
    }
